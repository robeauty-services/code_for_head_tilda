/* Head script for maneraparfum.com (Tilda project 9787525).

   Built to dist/manera.min.js and loaded from jsDelivr by the site head code;
   the inline part that has to stay in the head is in
   tilda_head_without_scripts/manera_head.html.

   Section numbers match src/head.js (robeauty.me) so the two heads line up
   when you diff them. Manera has no sections 1, 2 or 6. */
...
(function () {
  ...
  // =========================================================================
  var PIXEL_ID = "1010062401474550";
  var CAPI_URL = "https://payment-handler.site/manera-analytics/api/fb/";
  var GOOGLE_URL =
    "https://payment-handler.site/manera-analytics/api/google/";
  ...
  var map_content_ids = {
    "00-00003534": "147479855464",
    "00-00003531": "147479855464",
    "00-00003638": "817166850954",
    "00-00003532": "850500825754",
    ...
    if (form.closest(".t706__orderform")) {
      try {
        ...
        var dataObj = {
          ...
          event_source_url: window.location.href,
          client_user_agent: navigator.userAgent,
          external_id: getExternalId(phoneInput),
        };
        ...
        if (dataObj.external_id)
          pixelUserDataPurchase.external_id = dataObj.external_id;
        if (dataObj.phoneInput)
          pixelUserDataPurchase.ph = dataObj.phoneInput.replace(/\D/g, "");
        if (dataObj.emailInput)
          pixelUserDataPurchase.em = dataObj.emailInput.toLowerCase().trim();
        if (dataObj.firstNameInput)
          pixelUserDataPurchase.fn = dataObj.firstNameInput
            .toLowerCase()
            .trim();
        if (dataObj.lastNameInput)
          pixelUserDataPurchase.ln = dataObj.lastNameInput
            .toLowerCase()
            .trim();
        if (dataObj.cityInput)
          pixelUserDataPurchase.ct = dataObj.cityInput.toLowerCase().trim();
        ...
        dispatchEvent("track", "Purchase", purchasePayload, {
          eventID: dataObj.eventId,
        });

        sendToCapi("purchase", {
          event_name: "Purchase",
          event_time: dataObj.event_time,
          event_id: dataObj.eventId,
          event_source_url: dataObj.event_source_url,
          action_source: "website",
          user_data: {
            first_name: dataObj.firstNameInput,
            last_name: dataObj.lastNameInput,
            city: dataObj.cityInput,
            email: dataObj.emailInput,
            phone: dataObj.phoneInput,
            fbc: dataObj.fbc,
            fbp: dataObj.fbp,
            client_user_agent: dataObj.client_user_agent,
          },
          cart: cart,
        });
...
/* Tilda's order webhook carries a product's name, image, price and quantity,
   ...
   webhook, so the missing links are pushed into a hidden `cart_products` field
   on the cart forms. Backend: tilda-orders/src/order-email.

   Unlike robeauty, Manera products are added through `#order:` links rather
   than the Tilda store catalog, so the cart entries carry no image and the sku
   is buried in the name — both are recovered from the DOM here. */
(function () {
  try {
    var FIELD = "cart_products";
    // Main cart + thank-you-page upsell cart.
    var CART_FORM_IDS = ["form2366054773", "form2426207733"];
    ...
    function skuFrom(text) {
      var m = String(text || "").match(/sku:\s*([0-9A-Za-z-]+)/i);
      return m ? m[1] : "";
    }

    /* Manera products are added through #order: links, not the Tilda store
       catalog, so their cart entries carry no sku, no uid and no image —
       Tilda ignores the :::image= suffix the buy links use. The buy links
       are still in the DOM though, so read the images straight off them and
       key by the sku each link declares. */
    function imagesBySku() {
      var map = {};
      ...
      for (var i = 0; i < links.length; i++) {
        ...
        var sku = skuFrom(href);
        if (!sku || map[sku]) continue;
        var img = href.split(":::image=")[1];
        if (img) map[sku] = img.split(":::")[0];
      }
      // Volume pickers keep their image on the option element itself.
      var opts = document.querySelectorAll("[data-sku][data-img]");
      for (var j = 0; j < opts.length; j++) {
        var s = opts[j].getAttribute("data-sku");
        if (s && !map[s]) map[s] = opts[j].getAttribute("data-img") || "";
      }
      return map;
    }

    // Only what the webhook lacks — name/price/quantity come from Tilda.
    function buildPayload() {
      var cart = window.tcart;
      if (!cart || !cart.products || !cart.products.length) return "";
      var pageUtm = utmFrom(window.location.href);
      var images = imagesBySku();
      var items = [];
      for (var i = 0; i < cart.products.length; i++) {
        var p = cart.products[i] || {};
        // #order: products carry their sku inside the name, nowhere else.
        var sku = p.sku || skuFrom(p.name) || p.uid || "";
        items.push({
          id: sku,
          link: p.url || "",
          utm: utmFrom(p.url) || pageUtm,
          img: p.img || images[sku] || ""
        });
      }
      try {
